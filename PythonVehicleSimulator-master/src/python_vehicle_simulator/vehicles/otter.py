...
import numpy as np
import math
from python_vehicle_simulator.lib.control import PIDpolePlacement, PID
...
class otter:
    ...
    def __init__(
        ...
        if self.controlSystem == "headingAutopilot":
            self.controlDescription = (
                f"Heading autopilot, psi_d = {str(r)} deg"
                )
        elif self.controlSystem == "headingAutopilotPID":
            self.controlDescription = (
                f"Heading autopilot, psi_d = {str(r)} deg, PID parameters: Kp = {Kp}, Ki = {Ki}, Kd = {Ki}."
                )
        else:
            self.controlDescription = "Step inputs for n1 and n2"
            self.controlSystem = "stepInput"

        self.ref = r
        self.V_c = V_current
        self.beta_c = beta_current * D2R
        self.controlMode = self.controlSystem
    ...
    def headingAutopilot(self, eta, nu, sampleTime):
        ...
        if self.controlSystem == "headingAutopilot":
            ...
            [tau_N, self.e_int, self.psi_d, self.r_d, self.a_d] = PID(
                e_int=self.e_int,
                e_x=e_psi,
                e_v=e_r,
                x_d=self.psi_d,
